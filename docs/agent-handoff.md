...
- **A `.frame(height:)` shorter than a label's natural box does not centre text** —
  ...
  three times; don't.
- **Negative padding and negative stack spacing DO work**, and are the only levers
  for optical alignment and letter-spacing.
...
- **Text is sized to its ink, not the font's advance.** Splitting a string per
  ...
  Split on separators instead, keeping digit groups as real runs.
